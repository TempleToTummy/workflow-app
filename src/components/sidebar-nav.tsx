...
function NavLink({
  ...
  label: string;
  exact?: boolean;
  // Unread count. Zero renders nothing rather than a "0" pill — a badge that
...
export function SidebarNav({
  ...
  // The timer, if one is running. It lives here rather than on each page so a
  // clock started on a checklist step can be stopped from anywhere — which is
  // how a timer avoids being left running over a weekend.
  ...
  return (
    ...
    <aside className="no-print flex h-screen w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar-bg">
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto px-3 pb-4">
        ...
        <NavLink href="/mentions" label="Mentions" badge={unreadMentions} />
        ...
        {isAdmin && (
          <>
            ...
            <NavLink href="/workload" label="Workload" />
            <NavLink href="/requests" label="Waiting on clients" />
