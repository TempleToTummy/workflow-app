        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-line bg-black/[0.02] text-xs uppercase tracking-wide text-ink-muted">
              ...
              <th className="px-4 py-3 font-medium">Checklist Steps</th>
          ...
          <tbody>
            {projects.map((p) => (
              <tr key={p.id} className="border-b border-line last:border-0">
                ...
                <td className="px-4 py-3 text-ink-muted">
                  {p.subtasks.length === 0 ? (
                    <span className="text-overdue">Not configured</span>
                  ) : (
                    p.subtasks.length
                  )}
                </td>
                <td className="px-4 py-3 text-ink-muted">{p.clientAssignments.length}</td>
                <td className="px-4 py-3">
